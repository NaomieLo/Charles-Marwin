...
# Dummy sensor to simulate various elevations.
class DummySensor:
    @staticmethod
...
def test_consume_battery_flat():
    ...
    assert abs(motors.battery - expected) < 0.001, "Battery consumption on flat ground is incorrect"
    assert result is True, "consume_battery should return True when battery remains above 0"
...
def test_motor_start_stop():
    ...
    assert motors.is_running is True, "Motors is_running flag should be True after start"
    assert motors.charging_thread is not None, "Charging thread should be created after starting motors"
...
def test_charge_battery():
    ...
    assert abs(motors.battery - expected) < 0.001, "Battery charging did not work as expected"
