...
# Assume your module is imported as `database`
# And you want to intercept any calls to open('data/history.csv', ...)
# so that it uses a temporary file instead.

def custom_open(file, mode='r', *args, **kwargs):
    if file == "data/history.csv":
        # Redirect to your temporary file, e.g. self.history_csv_path
        return open(self.history_csv_path, mode, *args, **kwargs)
    return open(file, mode, *args, **kwargs)

with patch("builtins.open", side_effect=custom_open):
    # Call the function that writes history
