...
#         database.TOKEN_PATH = "data/token.json"
...
class TestDatabasePerformance(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with open(self.history_csv_path, "w", newline="") as csv_file:
            ...
            writer.writerow(["(32, 232)", "(4323, 232)", "robot1", "ai1", 120, 60, 1220])
            writer.writerow(["(32, 232)", "(4323, 232)", "robot1", "ai1", 120, 60, 1])
        # Set the module-level variable to point to our temporary file
    ...
    def test_update_history_performance(self):
        ...
        # Patch builtins.open so that any call to open("data/history.csv", ...) gets redirected to our temporary file.
        original_open = builtins.open  # Ensure this is defined in local scope
        ...
        with patch("builtins.open", side_effect=custom_open):
            ...
            for i in range(1000):
                new_data = [(i, i+1), (i+2, i+3), f"robot_updated{i}", f"ai_updated{i}", i*20, i*10, i]
                # Call update_history with appropriate parameters (adjust the signature if needed)
                database.update_history(i, new_data)
            duration = time.time() - start_time
