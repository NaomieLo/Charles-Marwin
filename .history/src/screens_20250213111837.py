...
class App(tk.Tk):
    def __init__(self):
        ...
        try:
            ...
            #self.welcome_bg = ImageTk.PhotoImage(self.original_welcome_bg)
        except Exception as e:
...
class WelcomeScreen(tk.Frame):
    def __init__(self, parent, controller, bg_image=None):
        super().__init__(parent)
        self.controller = controller
        self.original_bg = bg_image  # Store the original image
    ...
    def update_bg_image(self):
        ...
        if self.original_bg:
            win_width = self.winfo_width() or 800
            win_height = self.winfo_height() or 600

            # Resize image to fit the new window size
            resized_bg = self.original_bg.resize((win_width, win_height), Image.Resampling.LANCZOS)
