...
#         # Configure the grid so that frames expand with the window.
...
#         # Load welcome_bg using PIL for resizing support.
#         try:
...
#         if bg_image:
#             self.bg_image = bg_image  # initial image
#             # Create the image on the canvas and store its id
...
#             new_image = self.controller.welcome_bg_orig.resize((event.width, event.height), Image.ANTIALIAS)
...
class App(tk.Tk):
    def __init__(self):
        ...
        # Load the logo image both as a Tk image and a PIL image.
        try:
            self.logo_img = PhotoImage(file="images/logo.png")
        except Exception as e:
...
class WelcomeScreen(tk.Frame):
    def __init__(self, parent, controller, bg_image, logo_image):
        ...
        # Create a Canvas that will fill the entire frame.
        self.canvas = tk.Canvas(self, highlightthickness=0)
        self.canvas.pack(fill="both", expand=True)
        ...
        # Place the Start button (its position remains fixed here).
        start_button = tk.Button(
            self,
            text="Start",
            font=("Helvetica", 20),
    ...
    def _resize_bg(self, event):
        """Update the background image (using cover strategy) and logo when the canvas resizes."""
        # ---------------------
        ...
        if self.controller.welcome_bg_orig:
            ...
            resized_bg = self.controller.welcome_bg_orig.resize(new_size, Image.ANTIALIAS)
        ...
        if self.controller.logo_orig:
            ...
            resized_logo = self.controller.logo_orig.resize((new_logo_width, new_logo_height), Image.ANTIALIAS)
            ...
            logo_y = int(event.height * 0.10)
            # If already created, update its position and image.
...
class MainMenuScreen(tk.Frame):
    ...
    def _resize_logo(self, event):
        ...
        if self.controller.logo_orig:
            new_logo_width = int(event.width * 0.50)
            orig_logo_width, orig_logo_height = self.controller.logo_orig.size
            logo_ratio = orig_logo_height / orig_logo_width
            new_logo_height = int(new_logo_width * logo_ratio)
            resized_logo = self.controller.logo_orig.resize((new_logo_width, new_logo_height), Image.ANTIALIAS)
