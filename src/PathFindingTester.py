...
class PathFindingTester:
    def __init__(self,visualization):
        ...
        self.start = (0, 1)
        self.goal = (4, 2)
        self.expected = [(0, 1), (1, 1), (2, 2), (3, 3), (4, 4)]
    ...
    def print_map_with_path(self, elevation_map, path):
        ...
        vis_map = [[f"{elevation_map[i,j]:4d}" for j in range(cols)] for i in range(rows)]
        ...
        for i in range(len(path)-1):
            ...
            val = vis_map[curr_x][curr_y].strip()
            vis_map[curr_x][curr_y] = f"[{val}{symbol}]"
        
        # Mark final position
        last_x, last_y = path[-1]
        val = vis_map[last_x][last_y].strip()
        vis_map[last_x][last_y] = f"[{val}X]"
    ...
    def run_Astar_test(self):
        ...
        if self.visualization:
            if self.expected == None:
                print("There should not be any path found given this start and goal")
            else:
